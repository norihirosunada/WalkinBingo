...
class MainViewModel : ViewModel() {
    ...
    fun initBingo() {
        bingoCardRaw.clear()
        val dogLabel = BingoLabel(labels.indexOf("Dog"), "Dog")
        bingoCardRaw.add(dogLabel)
        bingoCardRaw.add(BingoLabel(labels.indexOf("Cat"), "Cat"))
        bingoCardRaw.add(BingoLabel(labels.indexOf("Bird"), "Bird"))
        labels.shuffled().subList(0, 6).forEach {
    ...
    fun isBingo(): Boolean {
        return bingoCardRaw.chunked(3).any { it.all { it.isHit.value == true } } ||
                bingoCardRaw.filterIndexed { index, bingoLabel -> index % 3 == 0 }.all { it.isHit.value == true } ||
                bingoCardRaw.filterIndexed { index, bingoLabel -> index % 3 == 1 }.all { it.isHit.value == true } ||
                bingoCardRaw.filterIndexed { index, bingoLabel -> index % 3 == 2 }.all { it.isHit.value == true } ||
                bingoCardRaw.filterIndexed { index, bingoLabel -> index % 4 == 0 }.all { it.isHit.value == true } ||
                bingoCardRaw.filterIndexed { index, bingoLabel -> index == 2 || index == 4 || index == 6}.all { it.isHit.value == true }
